...
SOCKS5h 代理：名单里的域名不走系统 DNS，改用 `cidr.txt` 里每个 `/24` 当 ECS 去查 `cname`，轮询解析出的边缘 IP 再拨号。
...
`cidr.txt` 一行一个 `/24` 网段，写成 `.0` 即可（`8.160.215.0` 或 `8.160.215.0/24` 都行）。`#` 开头是注释。

启动时用这些网段做 ECS 查 `cname` 填池；之后每 `refresh` 再查一遍并热替换。
...
- 匹配 `hosts` 的域名：忽略真实 DNS，从 ECS 解析出的 IP 轮询，保留客户端端口。
